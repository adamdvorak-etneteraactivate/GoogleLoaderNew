import pyodbc  # Replaced pymssql
import json
...
# --- Core Pipeline Logic Function (using PYODBC) ---
def run_full_load_pipeline(sql_id, sql_pwd, sql_server, sql_db, sql_schema, sql_table, gcs_bucket_name, bq_table_id_short, max_rows=None, replace_table=1):
    logging.info(f"Spouštím FULL pipeline z SQL do BQ: {bq_table_id_short}")

    # --- 1. Autorizace Google Cloud (using Environment Variable GCS_BQ_KEY_JSON) ---
    try:
        key_json_str = os.environ.get("GCS_BQ_KEY_JSON")
        if not key_json_str:
            raise ValueError("GCS_BQ_KEY_JSON environment variable not set.")
        ...
        logging.info(f"Úspěšně připojeno ke GCS/BQ Project ID: {gcs_client.project}")
    except Exception as e: 
        logging.error(f"Chyba pri autorizaci GCP: {e}"); return False
    ...
    try:
        ...
        # autodetect_schema remains True
    except Exception as e:
    ...
    conn = None 
    try:
        # Construct the connection string. NOTE: This uses the Azure (Linux) driver name.
        # You may need to change this line back to your local Windows driver name for local testing!
        conn_string = (
        ...
        if max_rows and int(max_rows) > 0: 
            sql_query = f"SELECT TOP {int(max_rows)} * FROM {sql_schema}.{sql_table}"
        else: 
            sql_query = f"SELECT * FROM {sql_schema}.{sql_table}"
    ...
    try:
        ...
        job_config = bigquery.LoadJobConfig(
            ...
            autodetect=autodetect_schema, # Use the autodetect flag determined earlier
            write_disposition=write_disposition,
        )
        # BQ will automatically create the table if create_disposition is not specified (default is CREATE_IF_NEEDED)
...
def HttpTriggerRunFullLoadPipeline(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        return func.HttpResponse(
             "Please pass a JSON object in the request body with the required parameters.",
             status_code=400
        )
    try:
        sql_id = req_body["sql_id"]
        sql_pwd = req_body["sql_pwd"]
        ...
        return func.HttpResponse(f"Missing required parameter in JSON body: {e}", status_code=400)
    except ValueError as e:
        return func.HttpResponse(f"Invalid value for max_rows or replace_table: {e}", status_code=400)

    success = run_full_load_pipeline(
        sql_id, sql_pwd, sql_server, sql_db, sql_schema, sql_table, 
